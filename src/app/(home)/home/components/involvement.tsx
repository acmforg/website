...
const Involvement = () => {
  return (
    <Maxwidth tw="flex flex-col items-center relative pt-6 gap-16 lg:(gap-20)">
      ...
      <div tw="flex flex-col gap-10 lg:(flex-row flex-wrap gap-10 justify-center)">
        {involvements.map(involvement => (
          <div
            key={involvement.title}
            css={[
              { background: `url(${involvement.bg})` },
              tw`bg-primary rounded-[1rem] px-6 py-[4rem] text-white text-center bg-center bg-cover flex flex-col items-center gap-6 flex-1 lg:(min-w-[30rem] max-w-[35rem])`,
            ]}
          >
